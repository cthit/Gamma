...
class ApiKeyConcurrencyIntegrationTest {
    @Test
    fun `concurrent rotations keep versions and cached credentials consistent`() {
        postgresEnvironmentForConcurrency().use { postgres ->
            databaseForConcurrency(postgres).use { database ->
                ...
                val rotation = RotateApiKey(database, cache, bcryptCost = 10, random = SequencedSecureRandom())
                val authenticator = ApiCredentialAuthenticator(database, cache)
                ...
                try {
                    ...
                    val replacements =
                        try {
                            val rotations =
                                List(2) { workers.submit<RawApiToken> { rotation.rotateForTest(database, id).token } }
                    ...
                    val winningTokens =
                        replacements.filter { token ->
                            run { authoritativeApi.authenticate(id, token, requiredType = ApiKeyType.ALLOW_LIST) } !=
                                null
                        }
                    assertEquals(1, winningTokens.size)
                    val winner = winningTokens.single()
                    val loser = replacements.single { it != winner }
                    assertNotNull(run { authenticator.authenticate(id, winner, requiredType = ApiKeyType.ALLOW_LIST) })
                    assertNull(run { authenticator.authenticate(id, loser, requiredType = ApiKeyType.ALLOW_LIST) })
                } finally {
                    blocker.rollback()
                    blocker.close()
                }
