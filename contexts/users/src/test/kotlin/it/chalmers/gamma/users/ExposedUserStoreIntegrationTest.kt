...
class ExposedUserStoreIntegrationTest {
    @Test
    fun `reads public administrative and API identity views`() =
        withUserDatabase { database ->
            val identities: UserStore = UserStore(database, BcryptPasswordHasher(cost = 10))
            ...
            run {
                ...
                assertEquals(michael.id, identities.findDirectoryUser(michael.id)?.id)
                ...
                assertEquals(michael.id, identities.apiUser(michael.id)?.id)
                assertTrue(checkNotNull(identities.apiUser(michael.id)).gdprTrained)
                assertTrue(identities.apiUsers().any { it.id == jim.id })
                assertTrue(identities.userExists(michael.cid))
                assertFalse(identities.userExists(Cid("nobody")))
                assertTrue(identities.isAdministrator(michael.id))
                assertTrue(identities.isGdprTrained(michael.id))
                assertFalse(identities.isAdministrator(jim.id))
                assertFalse(identities.isGdprTrained(jim.id))
                assertNull(identities.findDirectoryUser(UserId.generate()))
                assertNull(identities.apiUser(UserId.generate()))
            }
        }
    ...
    fun `directory pages are ordered bounded and hide locked users from ordinary viewers`() =
        withUserDatabase { database ->
            ...
            run {
                ...
                database.transaction { exec("UPDATE g_user SET locked = TRUE WHERE cid LIKE 'bulk%'") }
                val jim = assertNotNull(identities.findUser(Cid("jhalpert")))
...
private fun UserStore.directoryPage(
    query: String = "",
    afterCid: Cid? = null,
