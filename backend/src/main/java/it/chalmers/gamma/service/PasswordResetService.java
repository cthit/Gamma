...
public class PasswordResetService {
    ...
    public PasswordResetService(PasswordResetTokenRepository repository, ITUserService itUserService) {
    ...
    private void setToken(PasswordResetToken passwordResetToken, ITUserDTO userDTO, String token) {
        ITUser user = this.itUserService.getITUser(userDTO);
    ...
    public boolean userHasActiveReset(ITUserDTO user) {
        return this.repository.existsByItUser(this.itUserService.getITUser(user));
    }

    public boolean tokenMatchesUser(ITUserDTO user, String token) {
        PasswordResetToken storedToken = this.repository.findByItUser(this.itUserService.getITUser(user)).orElse(null);
    ...
    public void removeToken(ITUserDTO user) {
        this.repository.delete(Objects.requireNonNull(
                this.repository.findByItUser(this.itUserService.getITUser(user)).orElse(null)));
    }
