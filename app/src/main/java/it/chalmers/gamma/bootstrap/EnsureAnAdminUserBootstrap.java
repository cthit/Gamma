...
public class EnsureAnAdminUserBootstrap {
  ...
  public EnsureAnAdminUserBootstrap(
  ...
  public void ensureAnAdminUser() {
    ...
    if (this.userRepository.get(new Cid(admin)).isPresent()) {
      LOGGER.error(
          "There's no user that is admin right now, but there is a user that is named admin. "
    ...
    GammaUser adminUser =
        new GammaUser(
            ...
            Language.EN,
            new UserExtended(new Email(name + "@chalmers.it"), 0, false, null));

    try {
      this.userRepository.create(adminUser, new UnencryptedPassword(password));
