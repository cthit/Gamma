...
public class SettingsRepositoryAdapter implements SettingsRepository {
  ...
  private static final PersistenceErrorState superGroupTypeNotFound =
  ...
  public Settings getSettings() {
    SettingsEntity settingsEntity =
        repository.findTopByOrderByVersionDesc().orElseThrow(IllegalStateException::new);
  ...
  private SettingsEntity toEntity(Settings settings) {
    SettingsEntity settingsEntity =
        this.repository.findTopByOrderByVersionDesc().orElse(new SettingsEntity());
    ...
    settingsEntity.infoSuperGroupTypeEntities.addAll(
        settings.infoSuperGroupTypes().stream()
            .map(
                superGroupType ->
                    new SettingsInfoSuperGroupTypeEntity(
                        settingsEntity, new SuperGroupTypeEntity(superGroupType)))
            .toList());
