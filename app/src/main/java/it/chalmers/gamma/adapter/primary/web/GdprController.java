...
public class GdprController {
  ...
  @GetMapping("/gdpr")
  public ModelAndView getAlLGdprTrained(
  ...
  @PutMapping("/gdpr")
  public ModelAndView setGdprTrained(
      @RequestHeader(value = "HX-Request", required = false) boolean htmxRequest,
      @RequestParam Map<String, String> form) {
    List<UUID> gdprTrained = this.userGdprTrainingFacade.getGdprTrained();
    List<UUID> newGdprTrained =
        form.keySet().stream()
            .filter(s -> !("_csrf".equals(s) || "_method".equals(s)))
            .map(UUID::fromString)
            .toList();

    List<UUID> newlyTrained =
        newGdprTrained.stream().filter(userId -> !gdprTrained.contains(userId)).toList();
    List<UUID> noLongerTrained =
        gdprTrained.stream().filter(userId -> !newGdprTrained.contains(userId)).toList();
