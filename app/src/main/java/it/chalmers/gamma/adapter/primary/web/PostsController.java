...
public class PostsController {
  ...
  public ModelAndView updateEditPost(
    ...
    mv.addObject(
        "post",
        new PostFacade.PostDTO(
            postId, form.version + 1, form.svName, form.enName, form.emailPrefix));

    return mv;
  }

  public record UpdatePost(String svName, String enName, String emailPrefix, int version) {}

  @PostMapping("/posts")
  public ModelAndView createEditPost(
      @RequestHeader(value = "HX-Request", required = true) boolean htmxRequest, CreatePost form) {
    UUID postId =
        postFacade.create(new PostFacade.NewPost(form.svName, form.enName, form.emailPrefix));

    ModelAndView mv = new ModelAndView();

    mv.setViewName("partial/created-post");
    mv.addObject(
        "post", new PostFacade.PostDTO(postId, 0, form.svName, form.enName, form.emailPrefix));

    return mv;
  }

  public record CreatePost(String svName, String enName, String emailPrefix) {}
