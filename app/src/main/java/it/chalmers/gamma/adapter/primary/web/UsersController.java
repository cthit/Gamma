...
public class UsersController {
    ...
    public ModelAndView getUser(@RequestHeader(value = "HX-Request", required = false) boolean htmxRequest, @PathVariable("id") UUID userId) {
        ...
        if(AuthenticationExtractor.getAuthentication() instanceof UserAuthentication userAuthenticated) {
            if(userAuthenticated.isAdmin()) {
                Optional<UserFacade.UserExtendedWithGroupsDTO> userExtended = this.userFacade.getAsAdmin(userId);
    ...
    public record CreateUser(String password,
                             String nick,
                             String firstName,
                             String lastName,
                             String email,
    ...
    public ModelAndView getCreateUser(@RequestHeader(value = "HX-Request", required = false) boolean htmxRequest) {
        ModelAndView mv = new ModelAndView();
        ...
        mv.addObject("form", new CreateUser(
                "",
                "",
                "",
                "",
    ...
    @PostMapping("/users")
    public ModelAndView createUser(@RequestHeader(value = "HX-Request", required = false) boolean htmxRequest,
                                   CreateUser form) {
        try {
            UUID userId = this.userCreationFacade.createUser(new UserCreationFacade.NewUser(
                    form.password,
                    form.nick,
                    form.firstName,
                    form.lastName,
    ...
    public record EditUser(String nick,
                           String firstName,
    ...
    public ModelAndView getEditUser(@RequestHeader(value = "HX-Request", required = true) boolean htmxRequest,
        ...
        mv.addObject("form", new EditUser(
                u.nick(),
                u.firstName(),
                u.lastName(),
                u.email(),
    ...
    @PutMapping("/users/{id}")
    public ModelAndView editUser(@RequestHeader(value = "HX-Request", required = true) boolean htmxRequest,
                                 @PathVariable("id") UUID userId,
                                 EditUser form) {
        this.userFacade.updateUser(new UserFacade.UpdateUser(
                userId,
                form.nick,
                form.firstName,
                form.lastName,
