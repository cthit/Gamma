...
public class GdprTrainedRepositoryAdapter implements GdprTrainedRepository {
  ...
  public GdprTrainedRepositoryAdapter(
      GdprTrainedJpaRepository gdprTrainedJpaRepository, UserJpaRepository userJpaRepository) {
  ...
  public void setGdprTrainedStatus(UserId userId, boolean gdprTrained) {
    Optional<GdprTrainedEntity> maybeGdprTrainedEntity =
        this.gdprTrainedJpaRepository.findById(userId.value());

    if (gdprTrained && maybeGdprTrainedEntity.isEmpty()) {
      this.gdprTrainedJpaRepository.save(
          new GdprTrainedEntity(userJpaRepository.findById(userId.value()).orElseThrow()));
  ...
  public List<UserId> getAll() {
    return this.gdprTrainedJpaRepository.findAll().stream()
