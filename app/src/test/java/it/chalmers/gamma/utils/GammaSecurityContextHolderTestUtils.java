...
public class GammaSecurityContextHolderTestUtils {
  ...
  public static final GammaUser DEFAULT_USER =
      new GammaUser(
          ...
          Language.EN,
          new UserExtended(new Email("smurf@chalmers.it"), 0, false, ImageUri.defaultUserAvatar()));

  private static final ApiKey DEFAULT_CLIENT_API_KEY =
      new ApiKey(
          ApiKeyId.generate(),
          new PrettyName("Mat"),
          new Text("Api nyckel för mat", "Api key for mat"),
          ApiKeyType.CLIENT,
          ApiKeyToken.generate());

  public static final Client DEFAULT_CLIENT =
      new Client(
          ClientUid.generate(),
          ClientId.generate(),
          ...
          new PrettyName("Mat"),
          new Text("Klient för mat", "Client for mat"),
  ...
  public static GammaUser setAuthenticatedAsAdminUser(
  ...
  public static void setAuthenticatedUser(
    ...
    User user =
        new User(gammaUser.id().value().toString(), "{noop}" + password, Collections.emptyList());
    ...
    auth.setDetails(new UserAuthentication(gammaUser, isAdmin));
  }

  /** Will have no approved users */
  public static ClientWithApiKey setAuthenticatedAsClientWithApi(
  ...
  public static void setAuthenticatedAsClientWithApi(Client client) {
    ...
    context.setAuthentication(
        ApiAuthenticationToken.fromAuthenticatedApiKey(
