...
class ApiKeyFacadeUnitTest {
  ...
  @InjectMocks private ApiKeyFacade apiKeyFacade;
  ...
  /**
   * Creating a new type of api key should not be an easy decision. Make sure that you really need
   * to expose more data. Note that adding a new api key type means that you probably need to update
   * your user agreement.
   */
  @Test
  public void EnsureThereIsOnlyFourApiKeyTypes() {
    String[] expected = new String[] {"CLIENT", "GOLDAPPS", "INFO", "ALLOW_LIST"};

    assertThat(apiKeyFacade.getApiKeyTypes()).isEqualTo(expected);
  }

  /**
   * Main test for create() that checks that isAdmin is called and that the generated token is also
   * returned properly.
   */
  @Test
  public void Given_AValidApiKey_Expect_create_To_CreateValidApiKey()
      throws ApiKeyRepository.ApiKeyAlreadyExistRuntimeException {
    ApiKeyFacade.NewApiKey newApiKey =
        new ApiKeyFacade.NewApiKey(
            "My Api Key", "Det här är en test api nyckel", "This is my test api key", "CLIENT");
    ...
    assertThat(capturedApiKey.id()).isNotNull();
    assertThat(capturedApiKey.apiKeyToken()).isNotNull();

    // Makes sure that the token returned is the same that is sent to the repository
    assertThat(apiKeyTokenRaw).isEqualTo(capturedApiKey.apiKeyToken().value());
    ...
    assertThat(capturedApiKey).isEqualTo(expectedApiKey);
  ...
  public void Given_NullPrettyName_Expect_create_To_Throw() {
    ApiKeyFacade.NewApiKey newApiKey =
        new ApiKeyFacade.NewApiKey(
            null, "Det här är en test api nyckel", "This is my test api key", "CLIENT");
  ...
  public void Given_InvalidPrettyName_Expect_create_To_Throw() {
    ApiKeyFacade.NewApiKey newApiKey =
        new ApiKeyFacade.NewApiKey(
            "T", "Det här är en test api nyckel", "This is my test api key", "CLIENT");
  ...
  public void Given_NullText_Expect_create_To_Throw() {
    ApiKeyFacade.NewApiKey newApiKey =
        new ApiKeyFacade.NewApiKey("My Api Key", null, null, "CLIENT");
  ...
  public void Given_InvalidApiKeyType_Expect_create_ToThrow() {
    ApiKeyFacade.NewApiKey newApiKey =
        new ApiKeyFacade.NewApiKey(
            "My Api Key", "Det här är en test api nyckel", "This is my test api key", "LMAO");

    assertThatExceptionOfType(IllegalArgumentException.class)
        .isThrownBy(() -> apiKeyFacade.create(newApiKey));

    ApiKeyFacade.NewApiKey newApiKey2 =
        new ApiKeyFacade.NewApiKey(
            "My Api Key", "Det här är en test api nyckel", "This is my test api key", "client");
  ...
  @Test
  public void Given_InvalidApiKey_Expect_delete_To_Throw()
      throws ApiKeyRepository.ApiKeyNotFoundException {
    doThrow(ApiKeyRepository.ApiKeyNotFoundException.class).when(apiKeyRepository).delete(any());
  ...
  public void Given_AValidApiKeyId_Expect_getById_To_ReturnApiKey() {
    ...
    ApiKey apiKey =
        new ApiKey(
            ...
            new Text("Det här är en test api nyckel", "This is my test api key"),
            ApiKeyType.CLIENT,
            ApiKeyToken.generate());

    given(this.apiKeyRepository.getById(apiKey.id())).willReturn(Optional.of(apiKey));

    Optional<ApiKeyFacade.ApiKeyDTO> maybeApiKey = apiKeyFacade.getById(id);

    ApiKeyDTOAssert.assertThat(maybeApiKey).isEqualTo(apiKey);
  ...
  public void Given_InvalidApiKeyId_Expect_getById_To_ReturnEmptyOptional() {
    ...
    given(this.apiKeyRepository.getById(apiKeyId)).willReturn(Optional.empty());

    assertThat(this.apiKeyFacade.getById(id)).isEmpty();
  }
  ...
  public void Given_ExistingApiKeys_Expect_getAll_To_ThoseApiKeys() {
    ...
    ApiKey apiKey2 =
        new ApiKey(
            ...
            new Text("Det här är en test api nyckel 2", "This is my test api key 2"),
            ApiKeyType.INFO,
            ApiKeyToken.generate());

    List<ApiKey> apiKeys = List.of(apiKey1, apiKey2);

    given(this.apiKeyRepository.getAll()).willReturn(apiKeys);

    List<ApiKeyFacade.ApiKeyDTO> apiKeyDTOs = apiKeyFacade.getAll();

    assertThat(apiKeyDTOs.size()).isEqualTo(2);

    for (int i = 0; i < apiKeys.size(); i++) {
      ApiKeyFacade.ApiKeyDTO apiKeyDTO = apiKeyDTOs.get(i);
      ApiKey apiKey = apiKeys.get(i);

      ApiKeyDTOAssert.assertThat(apiKeyDTO).isEqualTo(apiKey);
    }
  ...
  @Test
  public void Given_AValidApiKeyId_Expect_resetApiKeyToken_To_ResetSuccessfully()
      throws ApiKeyFacade.ApiKeyNotFoundException,
          ApiKeyRepository.ApiKeyAlreadyExistRuntimeException,
          ApiKeyRepository.ApiKeyNotFoundException {
    ...
    given(this.apiKeyRepository.getById(apiKeyId)).willReturn(Optional.of(apiKey));
    ApiKeyToken generatedToken = ApiKeyToken.generate();
    given(this.apiKeyRepository.resetApiKeyToken(apiKeyId)).willReturn(generatedToken);

    String newToken = this.apiKeyFacade.resetApiKeyToken(apiKeyId.value());
    ApiKeyToken newApiKeyToken = new ApiKeyToken(newToken);

    assertThat(newApiKeyToken).isEqualTo(generatedToken);
  ...
  public static class ApiKeyDTOAssert
      extends AbstractAssert<ApiKeyDTOAssert, ApiKeyFacade.ApiKeyDTO> {
